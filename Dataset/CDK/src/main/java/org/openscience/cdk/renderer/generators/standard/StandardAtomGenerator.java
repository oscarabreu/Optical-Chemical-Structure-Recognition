...
import static org.openscience.cdk.renderer.generators.standard.HydrogenPosition.*;
...
final class StandardAtomGenerator {
     ...
     */
    private final static double DEFAULT_ADJUNCT_SPACING_RATIO = 0.12d;//控制间距
    private final static double DEFAULT_SUBSCRIPT_SIZE        = 0.8d;// 控制角标字体的大小
    ...
    AtomSymbol generatePseudoSymbol(String label, HydrogenPosition position) {
        ...
        final Font italicFont = font.deriveFont(Font.PLAIN);//控制取代基字体的，非常重要
//                                    .deriveFont(Font.ITALIC);
        ...
        if (pos > beg) {
            ...
            if (pos > beg) {
                ...
                outline = positionSuperscript(outlines.get(0), outline);//设置取代基为上标
//                outline = positionSubscript(outlines.get(0), outline);//设置取代基为下标
