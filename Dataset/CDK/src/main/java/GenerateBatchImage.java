...
public class GenerateBatchImage {
    public static void main(String[] args) throws Exception {
        ...
        String smilesFileName = "Single_Aromatic";
        String pngFileName = "/Users/oscar/Desktop/Temp/";
...
//            }
            ...
            if (smiles.contains("O[R1]") || smiles.contains("O[R2]") || smiles.contains("O[R5]") || smiles.contains("O[R6]")
                    || smiles.contains("O[P][G2]") || smiles.contains("O[R7]")) {

                smiles = smiles.replace("O[R1]", "[OR1]").replace("O[R2]", "[OR2]")
                        .replace("O[R5]", "[OR5]").replace("O[R6]", "[OR6]")
                        .replace("O[R7]", "[OR7]").replace("O[P][G2]", "[OPG2]");
            }
            //批量替换 临近取代基合并
            ...
            //根据1650个数据存在的规律，当出现以下几种字符时，图像会出现“*”。其实应该是ChemDraw软件识别不了，把未知原子转换成“*”了
...
//                System.out.println(numAdded);
               ...
               // .withAromaticDisplay()当在键上设置芳香性时，在图表中显示它。.withZoom 指定所需的缩放系数
                ...
                e.printStackTrace();
                System.out.println("Error processing line " + index + ": " + temp);
                errornum.add(temp);
            } finally {
                index++;
